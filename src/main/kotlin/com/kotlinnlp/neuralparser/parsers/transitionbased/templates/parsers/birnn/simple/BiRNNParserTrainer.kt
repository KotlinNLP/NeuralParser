...
class BiRNNParserTrainer<StateType : State<StateType>,
  ...
  private fun propagateErrors(context: TokensEmbeddingsContext) {
    ...
    this.neuralParser.biRNNEncoder.backward(outputErrors = context.items.map { it.errors?.array ?: this.zerosErrors })
    ...
    if (context.nullItemErrors != null) {
      ...
      this.neuralParser.paddingVectorEncoder.backward(outputErrors = listOf(context.nullItemErrors!!))
